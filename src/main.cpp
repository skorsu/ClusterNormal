...
// [[Rcpp::export]]
Rcpp::List SFCMM_realloc(arma::vec y, unsigned int K_max, double a0, double b0, 
                         double mu0, double s20, double xi0, arma::vec ci_init, 
                         arma::vec mu, arma::vec s2, arma::vec alpha_vec){
  
  /* Description: This function will perform the reallocation step, the first 
   *              step of the SFCMM. The different is we will sample the 
   *              parameters from prior if that cluster is inactive instead of 
   *              let them be 0.
   */
...
// [[Rcpp::export]]
Rcpp::List SFCMM_SM(arma::vec y, unsigned int K_max, double a0, double b0, 
                    double mu0, double s20, double xi0, arma::vec ci_init, 
                    arma::vec mu_init, arma::vec s2_init, arma::vec alpha_init,
                    int launch_iter){
  
  /* Description: This function will perform the split-merge MCMC, the second 
   *              step of the SFCMM. The main different from SFDMM is that we 
   *              can ignore the a_theta and b_theta.
   */
...
// [[Rcpp::export]]
Rcpp::List SFCMM_param(arma::vec clus_assign, arma::vec y, arma::vec mu, arma::vec s2, 
                       arma::vec alpha_vec, double old_U, unsigned int K_max, 
                       double a0, double b0, double mu0, double s20, double xi0){
  
  /* Description: This function will perform update the parameters in the model, 
   *              the third step of the SFCMM. The different is we will sample 
   *              the parameters from prior if that cluster is inactive instead of 
   *              let them be 0.
   */
...
// [[Rcpp::export]]
Rcpp::List SFCMM_model(int iter, unsigned int K_max, arma::vec init_assign, 
                       arma::vec y, double a0, double b0, double mu0, double s20, 
                       double xi0, int launch_iter, int print_iter){
  
  /* Description: This function will perform update the parameters in the model, 
   *              the third step of the SFCMM.
   */
  ...
  for(int i = 0; i < iter; ++i){
    // Reallocation Step
    Rcpp::List realloc_List = SFCMM_realloc(y, K_max, a0, b0, mu0, s20, xi0, 
                                            init_assign, mu, s2, init_alpha);
    arma::vec realloc_assign = realloc_List["new_ci"];
    arma::vec realloc_alpha = realloc_List["new_alpha"];
    arma::vec realloc_mu = realloc_List["new_mu"];
    ...
    // Split-Merge Step
    Rcpp::List sm_List = SFCMM_SM(y, K_max, a0, b0, mu0, s20, xi0, realloc_assign, 
                                  realloc_mu, realloc_s2, realloc_alpha,
                                  launch_iter);
    arma::vec sm_assign = sm_List["new_ci"];
    arma::vec sm_alpha = sm_List["new_alpha"];
    ...
    // Update the parameters
    Rcpp::List param_update_List = SFCMM_param(sm_assign, y, sm_mu, sm_s2, sm_alpha, 
                                               init_u, K_max, a0, b0, mu0, s20, xi0);
    arma::vec alpha_updated = param_update_List["new_alpha"];
    double U_update = param_update_List["new_U"];
    arma::vec assign_updated = param_update_List["new_ci"];
